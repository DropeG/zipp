...
from shared.meli_client import get_meli_headers, publish_meli_item, check_meli_item_exists, update_meli_item_description, update_meli_item_price_and_stock
...
def record_blocked_product(shopify_id, title, meli_item_id, final_item, reason):
    filename = BLOCKED_PRODUCTS_FILE
...
def upload_original_pictures_to_meli(image_urls):
    from shared.meli_client import get_meli_access_token
...
def run_publication(dry_run=True):
    ...
    blocked_products = load_json_object(BLOCKED_PRODUCTS_FILE)
