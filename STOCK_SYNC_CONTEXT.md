...
```bash
python3 scripts/process_shopify_to_meli_stock.py
```
...
```bash
python3 scripts/process_shopify_to_meli_stock.py --apply
```
...
```bash
./venv/bin/python scripts/process_meli_to_shopify_stock.py --check-permissions
./venv/bin/python scripts/process_meli_to_shopify_stock.py --limit 1
```
...
```text
Lee STOCK_SYNC_CONTEXT.md y continuemos desde Meli -> Shopify.
```
